...
## 1.0.1：出力前の不備を検出する追加条件

空所IDの数字は半角ASCIIとし、先頭に0を付けない。例えば `{{q021}}` や全角数字のマーカーは不正とする。空所記号はp1〜3の英語segment内だけで使用し、テーマ、題名、話者ラベル、メールの全ヘッダーと訳、解説、誤答理由には残さない。任意の題名欄も、存在する場合は非空の完成文にする。

`load_json` は最大8 MiBまでを読み込み、超過時は入力エラーとする。これは本実装の保護上限であって英検の仕様ではない。

`--answer-key` 成功時、標準出力は従来どおり解答のJSON配列。標準エラー出力には警告と意味・紙面の未検証状態をJSONで出す。解答表だけ取り出した際にも警告を見落とさないこと。構造エラー時は解答配列を出さず、エラーを返す。
